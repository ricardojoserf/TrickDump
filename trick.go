...
func Shock(proc_handle uintptr) ([]byte){
...
func createZipFile(lock_json []byte, shock_json []byte, barrel_json []byte, memFile_arr []MemFile, outputFileName string) error {
    // Create a new buffer to store the main zip file contents
    mainBuf := new(bytes.Buffer)
    mainZipWriter := zip.NewWriter(mainBuf)

    // Helper function to add a file to the main zip
    addFileToZip := func(zipWriter *zip.Writer, filename string, content []byte) error {
    ...
    // Add the JSON byte arrays to the main zip file
    if err := addFileToZip(mainZipWriter, "lock.json", lock_json); err != nil {
        return err
    }
    if err := addFileToZip(mainZipWriter, "shock.json", shock_json); err != nil {
        return err
    }
    if err := addFileToZip(mainZipWriter, "barrel.json", barrel_json); err != nil {
        return err
    }

    // Create a buffer for the nested zip
    nestedBuf := new(bytes.Buffer)
    nestedZipWriter := zip.NewWriter(nestedBuf)

    for _, memFile := range memFile_arr {
        fileWriter, err := nestedZipWriter.Create(memFile.Filename)
        if err != nil {
            fmt.Printf("[-] Error writing to ZIP file: %v\n", err)
        }
        _, _ = fileWriter.Write(memFile.Content)
    ...
    // Close the nested zip writer to finalize it
    if err := nestedZipWriter.Close(); err != nil {
        return err
    }

    // Add the nested zip as a file to the main zip
    if err := addFileToZip(mainZipWriter, "trick.zip", nestedBuf.Bytes()); err != nil {
        return err
    }

    // Close the main zip writer to finalize it
    if err := mainZipWriter.Close(); err != nil {
        return err
    }

    // Write the main zip buffer to a file
    err := os.WriteFile(outputFileName, mainBuf.Bytes(), 0644)
    if err != nil {
        return err
    }
...
func Barrel(proc_handle uintptr) ([]byte, []MemFile) {
    // Loop modules
...
func main() {
    ...
    lock_json := Lock()
    shock_json := Shock(proc_handle)
    barrel_json, memFile_arr := Barrel(proc_handle)

    err := createZipFile(lock_json, shock_json, barrel_json, memFile_arr, "trick.zip")
    if err != nil {
        fmt.Println("[-] Error creating zip file.")
        return
    }

    fmt.Println("[+] File trick.zip generated.")
}