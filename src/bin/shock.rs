...
use winapi::{
    ...
    um::{
        ...
        winnt::{ MEMORY_BASIC_INFORMATION, PAGE_NOACCESS, MEM_COMMIT, MAXIMUM_ALLOWED, TOKEN_ADJUST_PRIVILEGES, TOKEN_QUERY, },
    },
};
...
impl Default for ModuleInformation { fn default() -> Self { ModuleInformation { base_dll_name: [0; MAX_PATH], full_dll_path: [0; MAX_PATH], dll_base: ptr::null_mut(), size: 0 } } }
...
unsafe fn shock(filename: &str) -> Result<(), String> {
    ...
    match module_list {
        Ok(ref modules) => {
            ...
            // Create a string to hold all JSON objects
            ...
            for module in modules {
                ...
                // Create JSON object string manually
                let item = format!(
                    r#"{{"field0":"{}","field1":"{}","field2":"0x{:X}","field3":{}}},"#,
                ...
                json_output.push_str(&item);
            }

            // Remove trailing comma and wrap in array brackets
            let json_str = if !json_output.is_empty() {
        ...
        Err(e) => eprintln!("[-] Error enumerating modules: {}", e),
    }
...
struct Args {
    ...
    #[arg(short = 'j', long, default_value = "shock.json")]
    json_file: String,
}
...
fn main() {
    ...
    unsafe {
        ...
        if let Err(e) = shock(&args.json_file) {
            eprintln!("Error: {}", e);
        }
    }
}